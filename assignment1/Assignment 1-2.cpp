...
void testDeque1( unsigned int size )
{
   ...
   for( unsigned int k = 0; k < 4; k++ )
   {
      ...
      for( ; i <= 4 * ( size - 1 ) + k; i++ )
         data1.insert( data1.begin(), i );

      deque< int > data2( 1, 1 );
      unsigned int j;
      for( j = 2; j <= 4 * ( size / 2 - 1 ) + k; j++ )
         data2.insert( data2.end(), j );
      for( ; j <= 4 * ( size - 1 ) + k; j++ )
         data2.insert( data2.begin(), j );
...
void testDeque5()
{
   ...
   for( unsigned int i = 0; i < 1000; i++ )
   {
      ...
      case 0:
         ...
         if( rand() % 2 == 0 )
         {
            data1.insert( data1.begin(), value );
            data2.insert( data2.begin(), value );
         }
         else
         {
            data1.insert( data1.end(), value );
            data2.insert( data2.end(), value );
         }
         break;
      case 1:
         if( data1.size() > 0 )
