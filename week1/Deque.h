...
void deque< T >::insert( const iterator position, const T &val )
{
	...
	if (position.myOff - myOff <= mySize / 2) {//上半部
		...
		iterator before=begin()+last-myOff;
		iterator after=begin();

		for (int i=myOff;i< position.myOff;++i,++before, ++after)
			*before = *after;
		...
		*before = val;
		...
		iterator before = begin() + mySize -1;
  		for (int i = myOff+mySize ; i > position.myOff; --i,--after.myOff,--before.myOff)
			*after = *before;
...
{
		if (position.myOff - myOff < mySize / 2) {//上半
			iterator after = position;
			iterator before = position;
			int i;
			for (--before.myOff,i = position.myOff; i>myOff; --i,--after.myOff, --before.myOff) {
				*after = *before;
			}
			...
			iterator before = position;
			int i;
			for (++after.myOff,i=0; i<after.myOff-myOff-mySize;++i, ++before.myOff, ++after.myOff)
				*before = *after;
		}
		--mySize;
		if (mySize == 0)
